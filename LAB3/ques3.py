...
normal_dist = NormalDist(mean_weight, std_dev_weight)
...
data.append(['Probability that weight is less than 140 grams', p_less_140, round(normal_dist.cdf(140), 2)])

# ----------- Part B ---------------
percentile_top5 = np.percentile(distro, 95)
top_5 = [i for i in distro if i > percentile_top5]
data.append(['Lowest weight for premium product (top 5%)', round(min(top_5), 2), round(normal_dist.inv_cdf(0.95), 2)])

# ----------- Part C ---------------
percentile_lowest10 = np.percentile(distro, 10)
lowest_10 = [i for i in distro if i < percentile_lowest10]
data.append(['Highest Weight for defective product (lowest 10%)', round(max(lowest_10), 2), round(normal_dist.inv_cdf(0.10), 2)])
